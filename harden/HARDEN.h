...
#ifndef PROJECTS_HARDEN_HARDEN_H_
#define PROJECTS_HARDEN_HARDEN_H_

#define DEBUG_TYPE "HARDEN"
...
class HARDEN : public ModulePass {
...
public:
	...
	HARDEN() : ModulePass(ID) {BBCount = 0; passTime = 0; fixBranchCount = 0; splitBlockCount = 0;}
	...
	BBNode* insertBufferBlock(HARDEN::BBNode* pred, HARDEN::BBNode* succ);
	bool verifySignatures();
	bool shouldSkipBB(StringRef name);
	unsigned short calcSigDiff(HARDEN::BBNode* pn, HARDEN::BBNode* sn);
	void sigDiffGen();
	void printGraph();
...
char HARDEN::ID = 0;
static RegisterPass< HARDEN > X("HARDEN", "Control Flow checker", false, false);

#endif /* PROJECTS_HARDEN_HARDEN_H_ */
