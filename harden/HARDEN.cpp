...
void HARDEN::getAnalysisUsage(AnalysisUsage& AU) const {
	ModulePass::getAnalysisUsage(AU);
}

HARDEN::BBNode::BBNode(BasicBlock* no, int nu){
    node = no;
    num = nu;
    sig = 0;
    sigDiff = 0;
...
void HARDEN::BBNode::addEdge(BasicBlockEdge* e, int n){
	edges.push_back(e);
	edgeNums.push_back(n);    //this is the number of the node the edge goes to
}

void HARDEN::BBNode::removeEdge(int n){
	int i;
...
std::string HARDEN::BBNode::printNode(){
...
void HARDEN::insertErrorFunction(Module &M, StringRef name) {
...
void HARDEN::createErrorBlocks(Function &F) {
...
bool HARDEN::skipFnCl(Function* F){
...
bool HARDEN::shouldSkipF(StringRef name){
...
void HARDEN::populateGraph(Module &M){
	int i = 0;
...
void HARDEN::generateSignatures(){
...
int HARDEN::getIndex(BasicBlock* bb){
...
void HARDEN::checkBuffSig(BBNode* parent, BBNode* buff, BBNode* child){
...
void HARDEN::updateEdgeNums(BBNode* pred, BBNode* buff, BBNode* succ){
...
void HARDEN::updatePhiNodes(BBNode* pred, BBNode* buff, BBNode* succ){
...
void HARDEN::updateBranchInst(BBNode* pred, BBNode* buff, BBNode* succ){
...
unsigned short HARDEN::getSingleSig(){
...
HARDEN::BBNode* HARDEN::insertBufferBlock(HARDEN::BBNode* pred, HARDEN::BBNode* succ){
...
bool HARDEN::shouldSkipBB(StringRef name){
...
unsigned short HARDEN::calcSigDiff(HARDEN::BBNode* pred, HARDEN::BBNode* succ){
...
void HARDEN::printGraph(){
	for(BBNode* bn : graph){
		errs() << bn->printNode();
	}
}

GlobalVariable* HARDEN::setUpGlobal(Module &M, StringRef vName, IntegerType* IT1){
...
void HARDEN::insertStoreInsts(BBNode* bn, IntegerType* IT1, GlobalVariable* RTS,
...
void HARDEN::insertCompInsts(BBNode* bn, IntegerType* IT1, GlobalVariable* RTS,
...
Instruction* HARDEN::getInstructionBeforeOrAfter(Instruction* insertAfter, int steps){
...
Instruction* HARDEN::isInBB(BasicBlock* BB){
...
std::list<HARDEN::BBNode*> HARDEN::getRetBBs(Function* F){
	std::list<HARDEN::BBNode*> retBBs;
...
void HARDEN::updateCallInsts(CallInst* callI, BBNode* bn, IntegerType* IT1,
...
void HARDEN::verifyCallSignatures(IntegerType* IT1){
...
void HARDEN::updateRetInsts(IntegerType* IT1){
...
void HARDEN::splitBlocks(Instruction* I, BasicBlock* errBlock){
...
bool HARDEN::runOnModule(Module &M) {
	passTime = clock();
